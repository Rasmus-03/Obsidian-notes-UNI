...
> [!tip] For en 5-årig: Små skridt i mørke
> Forestil dig at du står i et mørkt rum og skal gå til døren. Du kan ikke se hele vejen på én gang — men du kan tage **små skridt** og mærke dig frem.
>
> Numeriske metoder (Euler, Heun) gør det samme med matematik: du kender hældningen lige **nu**, og du tager et lille skridt i den retning. Så gentager du — skridt for skridt — indtil du når frem.
>
> Egenværdier fortæller om systemet vil **falde til ro** (negativ λ) eller **eksplodere** (positiv λ) — ligesom et pendul der stopper med at svinge, hvis du ikke bliver ved med at skubbe.
...
> [!question]- Håndregning med Eulers metode
> Et lægemiddel nedbrydes i blodet med $dc/dt = -0,4c$. Startkoncentration $c(0) = 10$ µM. Brug Eulers metode med $\tau = 2$ til at beregne $c(2)$.
>
> **Løsning:**
>
> **1. Eulers formel:**
...
> **2. Indsæt værdier:**
> - $c_0 = 10$, $t_0 = 0$, $\tau = 2$
> - $f(0, 10) = -0,4 \cdot 10 = -4$
>
> $$ c_1 = 10 + 2 \cdot (-4) = 10 - 8 = 2 $$
>
> **Resultat:** $c(2) \approx 2$ µM.
>
> **3. Sammenlign med den analytiske løsning:**
> $dc/dt = -0,4c$ har løsningen $c(t) = 10 e^{-0,4t}$.
> $$ c(2) = 10 \cdot e^{-0,8} \approx 10 \cdot 0,449 = 4,49 \text{ µM} $$
>
> Euler med $\tau = 2$ giver $2$ µM — en fejl på $55\%$. Med $\tau = 0,5$ (som i R-eksemplet) får vi $c(2) = 10 \cdot (1 - 0,4 \cdot 0,5)^4 = 10 \cdot 0,8^4 = 4,096$ µM — fejl på kun $9\%$.
>
> > [!WARNING] τ's betydning
> > Jo større $\tau$, jo større fejl. For $dc/dt = -0,4c$ gælder reglen: vælg $\tau < 2/|k| = 5$ for stabilitet, og $\tau < 0,2/|k| = 0,5$ for god præcision.
